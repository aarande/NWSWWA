...
class NHCProduct(TextProduct):
    ...
    def get_jabbers(self, uri, uri2=None):
        # TODO pull out some of the NHC parsing from jabber into this class
        """ Get the jabber variant of this message """
        myurl = "%s?pid=%s" % (uri, self.get_product_id())

        tokens = re.findall(
            "(POST-TROPICAL CYCLONE|TROPICAL STORM|HURRICANE|TROPICAL DEPRESSION|REMNANTS OF) ([A-Z0-9\- ]*) (DISCUSSION|INTERMEDIATE ADVISORY|FORECAST/ADVISORY|ADVISORY) NUMBER\s+([0-9A-Z]+)",
            self.unixtext)
        if len(tokens) == 0:
